...
int main(int argc, char *argv[]) {
  ...
  uint32_t bitrate_max = atoi(argv[7]);
  uint32_t bitrate = bitrate_max;
  ...
  RTPSender rtp;
  rtp.init(localP,destIP,destP,pack.getClock(),bitrate_max,100);
  pack.init(fps,&rtp);
  ...
  while(1) {   
    ...
    if((num_frames % gop) == 0) {
        uint32_t new_bitrate = rtp.getRTPFriendlyRate(bitrate_max,time_inicio);
